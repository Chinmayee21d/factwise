/* ============================================================
 * CollaborationAnimation.jsx  (browser-preview version)
 * FactWise — "Collaborate Seamlessly · Every Team. Every Vendor. One Platform."
 *
 * Visualises the FULL procurement collaboration flow inside one platform:
 *   1. REQUEST    — Priya creates REQ-417 (Requisitioner module pulses).
 *   2. ROUTED     — auto-routes to Sourcing (no email handoff).
 *   3. RFQ        — Raj invites 3 vendors. Vendor strip lights up.
 *   4. BIDS       — vendors submit on platform. Bids stream back in.
 *   5. NEGOTIATE  — Autobot runs counter-offers with ACME.
 *   6. APPROVE    — Vikram approves the award, in context.
 *   7. PO         — Anika issues PO-882 to ACME.
 *   8. INVOICE    — ACME raises invoice on platform.
 *   9. PAYMENT    — Finance releases payment. Loop.
 *
 * Each phase lights up the relevant modules, draws flow arrows / packets,
 * morphs the centre "active workspace" to show the current artifact, and
 * appends a sentence to the live activity feed.
 *
 * Usage in HTML preview:
 *   <script type="text/babel" src="CollaborationAnimation.jsx"></script>
 *   ReactDOM.createRoot(el).render(<CollaborationAnimation speed={1} />)
 *
 * Port to Next.js / TypeScript:
 *   1. Rename to .tsx, add `'use client'` at the top.
 *   2. Replace `const { useState, useEffect, useRef } = React` with named imports.
 *   3. Replace `window.CollaborationAnimation = …` with `export default …`.
 *   4. CSS is scoped under `co-*` — keep inject, or move to .module.css.
 * ============================================================ */

const { useState: coUseState, useEffect: coUseEffect, useRef: coUseRef } = React;

/* ===================== ICONS ===================== */
const COI = {
  Clipboard: (p) => <svg viewBox="0 0 24 24" width={p.s||14} height={p.s||14} fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="8" y="2" width="8" height="4" rx="1"/><path d="M16 4h2a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h2"/><path d="M9 12h6M9 16h4"/></svg>,
  ...
  Shield:    (p) => <svg viewBox="0 0 24 24" width={p.s||14} height={p.s||14} fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/><polyline points="9 12 11 14 15 10"/></svg>,
  File:      (p) => <svg viewBox="0 0 24 24" width={p.s||14} height={p.s||14} fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>,
  Wallet:    (p) => <svg viewBox="0 0 24 24" width={p.s||14} height={p.s||14} fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 12V7H5a2 2 0 0 1 0-4h14v4"/><path d="M3 5v14a2 2 0 0 0 2 2h16v-5"/><path d="M18 12a2 2 0 0 0 0 4h4v-4z"/></svg>,
  Building:  (p) => <svg viewBox="0 0 24 24" width={p.s||14} height={p.s||14} fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="4" y="2" width="16" height="20" rx="2"/><path d="M9 22v-4h6v4M8 6h.01M16 6h.01M12 6h.01M8 10h.01M16 10h.01M12 10h.01M8 14h.01M16 14h.01M12 14h.01"/></svg>,
  ...
  Bolt:      (p) => <svg viewBox="0 0 24 24" width={p.s||10} height={p.s||10} fill="currentColor"><path d="M13 2L4 14h7l-2 8 9-12h-7l2-8z"/></svg>,
  Radio:     (p) => <svg viewBox="0 0 24 24" width={p.s||10} height={p.s||10} fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="2"/><path d="M16.24 7.76a6 6 0 0 1 0 8.48M7.76 16.24a6 6 0 0 1 0-8.48M19.07 4.93a10 10 0 0 1 0 14.14M4.93 19.07a10 10 0 0 1 0-14.14"/></svg>,
  Arrow:     (p) => <svg viewBox="0 0 24 24" width={p.s||10} height={p.s||10} fill="none" stroke="currentColor" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round"><line x1="5" y1="12" x2="19" y2="12"/><polyline points="12 5 19 12 12 19"/></svg>,
  Bot:       (p) => <svg viewBox="0 0 24 24" width={p.s||12} height={p.s||12} fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="8" width="18" height="12" rx="2"/><path d="M12 4v4M8 2v2M16 2v2"/><circle cx="9" cy="14" r="1"/><circle cx="15" cy="14" r="1"/></svg>,
};

/* ===================== DATA ===================== */

// 5 internal modules — left column, top→bottom
const CO_MODULES = [
...
// 3 external vendors — right column
const CO_VENDORS = [
...
// PHASES — each lights modules / vendors, draws edges, sets centre workspace
const CO_PHASES = [
  {
    id: 'request',
    label: 'Request',
    ...
    edges: [],
    panel: 'req',
    activity: 'Priya created REQ-417 · 12 line items',
    badge: '01',
  },
  {
    id: 'route',
    label: 'Routed',
    lit: ['priya', 'raj'], vendors: [],
    edges: [['priya', 'raj']],
    panel: 'req',
    activity: 'Auto-routed to Sourcing — no email, no handoff',
    badge: '02',
  },
  {
    id: 'rfq',
    label: 'RFQ launched',
    lit: ['raj'], vendors: ['acme', 'titan', 'apex'],
    edges: [['raj', 'v:acme'], ['raj', 'v:titan'], ['raj', 'v:apex']],
    panel: 'rfq',
    activity: 'Raj invited 3 vendors to RFQ-208',
    badge: '03',
  },
  {
    id: 'bids',
    ...
    panel: 'bids',
    activity: 'ACME bid received · −3.2% vs target price',
    badge: '04',
  },
  {
    id: 'negotiate',
    label: 'Negotiate',
    lit: ['raj'], vendors: ['acme'],
    edges: [['raj', 'v:acme'], ['v:acme', 'raj']],
    panel: 'negotiate',
    activity: 'FW Autobot · counter-offer to ACME · target −5%',
    badge: '05',
  },
  {
    id: 'approve',
    label: 'Approve',
    lit: ['raj', 'vikram'], vendors: ['acme'],
    edges: [['raj', 'vikram']],
    panel: 'approve',
    activity: 'Vikram approved the award · ₹4.82L · ACME Mfg',
    badge: '06',
  },
  {
    id: 'po',
    label: 'PO issued',
    lit: ['anika'], vendors: ['acme'],
    edges: [['anika', 'v:acme']],
    panel: 'po',
    activity: 'Anika issued PO-882 to ACME · terms locked',
    badge: '07',
  },
  {
    id: 'invoice',
    label: 'Invoice',
    lit: ['finance'], vendors: ['acme'],
    edges: [['v:acme', 'finance']],
    panel: 'invoice',
    activity: 'ACME raised INV-119 on platform · 3-way matched',
    badge: '08',
  },
  {
    id: 'pay',
    ...
    panel: 'pay',
    activity: 'Meera released payment · ACME notified instantly',
    badge: '09',
  },
];
...
.co-root { position: relative; width: 100%; height: 560px; font-family: 'Inter', system-ui, sans-serif;
  color: #0b1322; background:
    radial-gradient(900px 500px at 80% 0%, rgba(54,102,255,0.05), transparent 60%),
    radial-gradient(700px 500px at 0% 100%, rgba(0,184,132,0.06), transparent 60%),
    #fafbff;
...
.co-chrome-live .pulse::after { content: ""; position: absolute; inset: 0; border-radius: 50%;
  background: #00b884; box-shadow: 0 0 0 0 rgba(0,184,132,0.5);
...
.co-rail-step.active { background: #0b1322; color: white;
  box-shadow: 0 2px 8px -2px rgba(11,19,34,0.3); }
...
/* ───── Main grid: modules | workspace | vendors ───── */
...
/* Internal team modules column */
.co-col-mod { display: flex; flex-direction: column; gap: 8px; padding-right: 4px; z-index: 2; position: relative; }
.co-col-head { display: flex; align-items: center; justify-content: space-between;
  padding: 0 4px 4px; }
...
.co-col-head .b { display: inline-flex; align-items: center; gap: 4px;
  ...
  font-family: 'JetBrains Mono', ui-monospace, monospace; font-size: 8px; font-weight: 700;
  color: #3666ff; }
...
.co-mod .info .r .lock { color: #cbd5e1; }
...
.co-ws-tab { padding: 5px 9px; border-radius: 7px 7px 0 0; font-size: 10px; font-weight: 600;
  ...
  transition: all 0.3s; }
.co-ws-tab.active { background: white; color: #0b1322;
  border-color: #f1f5f9; }
...
/* Negotiate panel — message bubbles */
...
.co-app .stamp-ok { position: absolute; top: 36px; right: 16px;
  padding: 4px 10px; border: 1.5px solid #7c3aed; color: #7c3aed; border-radius: 6px;
  font-family: 'JetBrains Mono', ui-monospace, monospace; font-size: 10px; font-weight: 800;
  letter-spacing: 0.12em; transform: rotate(-8deg); opacity: 0;
  animation: co-stamp 0.6s 0.4s cubic-bezier(0.34,1.56,0.64,1) forwards; }
...
.co-rfq-card.sent { background: white; }
...
.co-col-ven .head { display: flex; align-items: center; justify-content: space-between;
  padding: 0 4px 4px; }
...
.co-ven.lit .row2 .d { background: var(--ven-tone, #3666ff);
  animation: co-blink 1.6s ease-out infinite; }

/* Vendor strip header pill */
...
/* Activity feed */
.co-feed { padding: 9px 14px 11px; background: #0b1322; color: white;
...
.co-feed-item .d { width: 4px; height: 4px; border-radius: 50%; background: #475569; flex-shrink: 0; }
.co-feed-item.head-item .d { background: #00d196; box-shadow: 0 0 0 3px rgba(0,209,150,0.18); }
.co-feed-item.head-item { color: white; font-weight: 500; }
.co-feed-item:not(.head-item) { color: #64748b; font-size: 10px; }

/* Packet */
.co-packet { position: absolute; width: 12px; height: 12px; border-radius: 50%;
  pointer-events: none; z-index: 4; }
.co-packet .core { width: 100%; height: 100%; border-radius: 50%;
  background: radial-gradient(circle, white 0%, var(--p-tone, #3666ff) 70%);
  box-shadow: 0 0 12px var(--p-tone, #3666ff), 0 0 24px var(--p-tone, #3666ff); }
`;

/* ===================== HELPERS ===================== */

function coGetEndpoint(target, rects, root) {
  // target: module id, "v:<vendor>", or "ws" centre
  if (!root) return null;
  let el;
  if (target === 'ws') {
    el = root.querySelector('.co-ws');
  } else if (target.startsWith('v:')) {
    el = root.querySelector(`[data-ven="${target.slice(2)}"]`);
  } else {
    el = root.querySelector(`[data-mod="${target}"]`);
  }
  if (!el) return null;
...
function CollaborationAnimation(props) {
  const speed = props.speed || 1;
  const [phaseIdx, setPhaseIdx] = coUseState(0);
  const [feed, setFeed] = coUseState([CO_PHASES[0].activity]);
  const [tick, setTick] = coUseState(0);
  const rootRef = coUseRef(null);

  // Inject style once
  coUseEffect(() => {
    if (!document.getElementById('co-style')) {
      const s = document.createElement('style');
      s.id = 'co-style'; s.textContent = CO_STYLE;
  ...
  // Append to feed on phase change (newest first, keep 4)
  coUseEffect(() => {
    setFeed((prev) => {
  ...
  // Force re-render for packet position (using key tied to phase)
  coUseEffect(() => {
  ...
  return (
    <div className="co-root" ref={rootRef}>
      {/* Chrome */}
      <div className="co-chrome">
        ...
        <div className="co-chrome-url">
          <svg viewBox="0 0 24 24" width="10" height="10" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>
          app.factwise.io · One workspace
        </div>
        <div className="co-chrome-spacer"></div>
      ...
      <div className="co-grid">
        ...
        <div className="co-col-mod">
          ...
          {CO_MODULES.map((m) => {
            ...
            return (
              <div key={m.id} className={`co-mod ${lit ? 'lit' : ''}`} data-mod={m.id}
                   style={{
                     boxShadow: lit
                ...
                <div className="icon-wrap" style={lit ? {background:`${m.tone}18`, color:m.tone} : {}}>
                  <Icon s={11}/>
                </div>
        ...
        <div className="co-col-ws">
          <div className="co-ws">
            <div className="co-ws-tabs" style={{'--tab-tone': '#3666ff'}}>
              <div className={`co-ws-tab ${['req','route'].includes(phase.id) ? 'active' : ''}`}><span className="d"/>REQ-417</div>
              <div className={`co-ws-tab ${['rfq','bids','negotiate'].includes(phase.id) ? 'active' : ''}`} style={{'--tab-tone':'#0d9488'}}><span className="d"/>RFQ-208</div>
              <div className={`co-ws-tab ${phase.id === 'approve' ? 'active' : ''}`} style={{'--tab-tone':'#7c3aed'}}><span className="d"/>APR-441</div>
              <div className={`co-ws-tab ${phase.id === 'po' ? 'active' : ''}`} style={{'--tab-tone':'#f59e0b'}}><span className="d"/>PO-882</div>
              <div className={`co-ws-tab ${['invoice','pay'].includes(phase.id) ? 'active' : ''}`} style={{'--tab-tone':'#10b981'}}><span className="d"/>INV-119</div>
            </div>
        ...
        <div className="co-col-ven">
          ...
          {CO_VENDORS.map((v) => {
            ...
            const stStr = phase.id === 'rfq' ? 'Invited'
                       ...
                       : 'Idle';
            return (
              <div key={v.id} className={`co-ven ${lit ? 'lit' : ''}`} data-ven={v.id}
                   style={{
          ...
          <div style={{flex:1}}/>
          <div style={{padding:'7px 8px', background:'rgba(54,102,255,0.06)', borderRadius:8,
...
function ReqPanel({phase}) {
  const show = phase.id === 'req' || phase.id === 'route';
  return (
    <div className={`co-panel ${show ? 'show' : ''}`}>
      <div className="co-panel-head">
        ...
        <div className="stamp" style={{background: phase.id==='route' ? '#ecfdf5' : '#eff4ff',
      ...
      {[
        ...
        <div key={i} className={`co-li ${i < (show ? 6 : 0) ? 'ok' : ''}`}>
  ...
  return (
    <div className={`co-panel ${show ? 'show' : ''}`}>
      ...
      <div className="co-bid-row head">
        <span/><span>Vendor</span><span style={{textAlign:'right'}}>Landed</span>
        <span style={{textAlign:'right'}}>Δ Target</span><span/>
      </div>
      {[
        {v: CO_VENDORS[0], price:'₹4.79L', delta:'−3.2%', neg: true, win: true},
        {v: CO_VENDORS[1], price:'₹5.00L', delta:'+1.1%', neg: false},
        {v: CO_VENDORS[2], price:'₹5.15L', delta:'+4.0%', neg: false},
      ].map((b, i) => (
      ...
      <div style={{marginTop:8, padding:'5px 7px', background:'#f9fafc', borderRadius:6,
...
function NegotiatePanel({phase}) {
  ...
  return (
    <div className={`co-panel ${show ? 'show' : ''}`}>
      ...
      <div style={{marginTop:6, padding:'5px 8px', background:'#ecfdf5', borderRadius:6,
  ...
  return (
    <div className={`co-panel ${show ? 'show' : ''}`}>
      ...
      <div className="co-app" style={{position:'relative'}}>
        ...
        {show && <div className="co-app-stamp co-app-fix"></div>}
      </div>
      {show && (
        <div style={{position:'absolute', top:42, right:8, padding:'4px 10px',
      ...
      <div className="co-po">
        ...
        <div className="grid2">
          <div>
            ...
            <div className="v">ACME Mfg</div>
          </div>
          <div>
            <div className="k">Total</div>
            <div className="v">₹4,68,000</div>
          </div>
          <div>
            <div className="k">Delivery</div>
            <div className="v">Mumbai · 14 days</div>
          </div>
          <div>
            <div className="k">Payment terms</div>
            <div className="v">Net 30 · 3-way match</div>
          </div>
        </div>
        <div className="sig">
  ...
  return (
    <div className={`co-panel ${show ? 'show' : ''}`}>
      ...
      <div className="co-inv">
        ...
        <div style={{marginTop:7, padding:'6px 8px', background:'#fafbff', borderRadius:6,
...
function PayPanel({phase}) {
  ...
  return (
    <div className={`co-panel ${show ? 'show' : ''}`}>
      ...
      <div style={{marginTop:9, padding:'7px 9px', background:'#f9fafc', borderRadius:7,
                   display:'flex', justifyContent:'space-between', alignItems:'center',
      ...
      <div style={{marginTop:6, padding:'7px 9px', background:'#0b1322', borderRadius:7,
                   display:'flex', justifyContent:'space-between', alignItems:'center',
                   fontSize:9.5, color:'#94a3b8'}}>
        <span>Every step · every stakeholder · one platform</span>
        <span style={{color:'#00d196', fontFamily:"'JetBrains Mono', monospace", fontWeight:700, letterSpacing:'0.06em'}}>↻ loop</span>
      </div>
    </div>
  );
}
...
function FlowOverlay({phase, tick, rootRef}) {
  const [pts, setPts] = coUseState([]);

  coUseEffect(() => {
    if (!rootRef.current) return;
    const newPts = phase.edges.map(([from, to]) => {
      const a = coGetEndpoint(from, null, rootRef.current);
      const b = coGetEndpoint(to, null, rootRef.current);
      // Adjust to grid coords (subtract grid offset)
      const grid = rootRef.current.querySelector('.co-grid');
      ...
      // Pull endpoints toward the workspace edges to feel like they connect to the centre panel.
      let ax = a.x - offX, ay = a.y - offY;
      let bx = b.x - offX, by = b.y - offY;
      // Inset endpoints to module edge instead of centre
      const fromIsV = from.startsWith('v:');
      const toIsV   = to.startsWith('v:');
      // crude horizontal inset
      if (!fromIsV) ax += 80;    // module right edge
      else          ax -= 70;    // vendor left edge
      if (!toIsV)   bx -= 80;
      else          bx += 70;
  ...
  // Choose packet tone based on direction (module→vendor uses module tone, etc.)
  const tone = (() => {
  ...
  return (
    <svg className="co-flow" preserveAspectRatio="none"
...
/* ===================== REGISTER ===================== */
window.CollaborationAnimation = CollaborationAnimation;
