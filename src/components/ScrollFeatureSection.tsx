  ...
  return (
    <div style={{ display: 'flex', width: '100%', height: '100%', background: '#0a0a0c' }}>
      ...
      <div style={{ 
        ...
        {[1, 2, 3, 4, 5].map(i => (
          <div key={i} style={{ width: 32, height: 32, borderRadius: 10, background: i === 1 ? 'rgba(255,255,255,0.1)' : 'transparent', border: i === 1 ? '1px solid rgba(255,255,255,0.1)' : 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, color: 'rgba(255,255,255,0.2)' }}>
            {i === 1 ? '◍' : '○'}
          </div>
        ))}
      </div>
...
const rfqRows = [
  ...
  { name: 'Logistics RFP',   bids: '5 bids',  due: 'Apr 30', status: 'Award Stage', color: '#7c5cfc' },
  { name: 'Software License',bids: '3 bids',  due: 'May 10', status: 'Approval Pending',  color: '#f59e0b' },
];

function SourcingPanel() {
  return (
    <DashboardShell title="Strategic Sourcing · RFQ #8421" activeColor="#7c5cfc">
      <div style={{ padding: '24px', height: '100%', overflow: 'hidden' }}>
        ...
        <div style={{ flex: 1.5, paddingTop: 10 }}>
                {rfqRows.map((row, i) => (
                  ...
                  >
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                      <div style={{ width: 24, height: 24, borderRadius: 4, background: 'rgba(255,255,255,0.05)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, color: '#6b6b7a' }}>#{i+1}</div>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                        <span style={{ fontSize: 13, fontWeight: 600, color: '#f4f4f5' }}>{row.name}</span>
                ...
                <div style={{ marginTop: 24, padding: 16, borderRadius: 12, background: 'rgba(52,211,153,0.03)', border: '1px solid rgba(52,211,153,0.1)' }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
                     {[
                       ...
                       <div key={v.vendor} style={{ padding: 10, borderRadius: 8, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.05)' }}>
                         <div style={{ fontSize: 10, color: '#f4f4f5', marginBottom: 4, fontWeight: 600 }}>{v.vendor}</div>
                         <div style={{ fontSize: 14, fontWeight: 700, color: '#34d399' }}>{v.share}</div>
                         <div style={{ fontSize: 10, color: '#52525b' }}>{v.cost}</div>
                       </div>
                     ))}
                     <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', border: '1px dashed rgba(255,255,255,0.1)', borderRadius: 8, fontSize: 10, color: '#52525b', gap: 4 }}>
                       <span>+ Award</span>
                       <span>Alternate</span>
                     </div>
                  </div>
          ...
          {/* Side Info */}
          <div style={{ flex: 1, background: 'rgba(255,255,255,0.015)', borderRadius: 16, padding: 20, border: '1px solid rgba(255,255,255,0.05)' }}>
            ...
            <div style={{ padding: '16px', borderRadius: 12, background: 'rgba(52,211,153,0.05)', border: '1px solid rgba(52,211,153,0.15)', marginBottom: 20 }}>
              ...
              <span style={{ fontSize: 14, fontWeight: 700, color: '#f4f4f5' }}>Alpha Steel Inc.</span>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {[
                ...
                <div key={item.label} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <span style={{ fontSize: 12, fontWeight: 600, color: item.color || '#f4f4f5' }}>{item.val}</span>
                </div>
              ))}
            </div>
          </div>
...
const spendCats = [
  { label: 'Direct Materials', pct: 42, color: '#7c5cfc' },
...
const kpiCards = [
  { label: 'Total Spend YTD', value: '$4.2M',      valueColor: '#f4f4f5', bg: 'rgba(255,255,255,0.03)', border: 'rgba(255,255,255,0.07)' },
    ...
    <DashboardShell title="Spend Intelligence · Dashboard" activeColor="#34d399">
      <div style={{ padding: '24px', height: '100%', overflow: 'hidden' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16, marginBottom: 28 }}>
          {[
            { label: 'Total Spend', val: '$4.2M', delta: '+12%', color: '#f4f4f5' },
              ...
              {spendCats.map((cat, i) => (
                <div key={cat.label}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                    ...
                    <span style={{ fontSize: 12, fontWeight: 600, color: '#f4f4f5' }}>{cat.pct}%</span>
                  </div>
        ...
        <div style={{ display: 'flex', gap: 20, height: '100%' }}>
          <div style={{ flex: 1, paddingTop: 10 }}>
            <div style={{ display: 'flex', gap: 8, marginBottom: 20 }}>
              {filterTabs.map(f => (
                <div key={f} style={{ fontSize: 11, padding: '6px 12px', borderRadius: 8, color: f === 'All' ? '#f4f4f5' : '#52525b', background: f === 'All' ? 'rgba(255,255,255,0.08)' : 'transparent', border: '1px solid rgba(255,255,255,0.08)' }}>{f}</div>
              ))}
            </div>
          ...
          <div style={{ flex: 0.8, background: 'rgba(255,255,255,0.015)', borderRadius: 16, padding: 20, border: '1px solid rgba(255,255,255,0.05)' }}>
            <div style={{ width: '100%', height: 120, borderRadius: 12, background: 'rgba(255,255,255,0.03)', marginBottom: 20, border: '1px solid rgba(255,255,255,0.05)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
               ...
               <div style={{ height: 32, borderRadius: 8, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, color: '#6b6b7a' }}>View Documents</div>
            </div>
          </div>
        </div>
      </div>
              ...
              >
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    <span style={{ fontSize: 13, fontWeight: 600, color: '#f4f4f5' }}>{c.name}</span>
                    <span style={{ fontSize: 9, padding: '2px 6px', borderRadius: 4, background: 'rgba(255,255,255,0.05)', color: '#52525b' }}>{c.tiers}</span>
                  </div>
             ...
             <span style={{ fontSize: 11, fontWeight: 700, color: '#52525b', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 16, display: 'block' }}>Contract Repository</span>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                 {[
                   ...
                   <div key={doc.label} style={{ padding: '12px', borderRadius: 10, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)' }}>
                     <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
                       <span style={{ fontSize: 12, color: '#f4f4f5', fontWeight: 500 }}>{doc.label}</span>
                       <span style={{ fontSize: 9, padding: '2px 6px', borderRadius: 4, background: doc.status === 'Verified' ? 'rgba(52,211,153,0.1)' : 'rgba(255,255,255,0.05)', color: doc.status === 'Verified' ? '#34d399' : '#a1a1aa' }}>{doc.status}</span>
                     </div>
                 ...
                 <div style={{ marginTop: 12, display: 'flex', alignItems: 'center', gap: 12, padding: 12, borderRadius: 10, background: 'rgba(255,255,255,0.02)', border: '1px dashed rgba(255,255,255,0.1)' }}>
                    ...
                    <div>
                      <div style={{ fontSize: 10, color: '#a1a1aa', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Asset Tracking</div>
                      <div style={{ fontSize: 11, color: '#f4f4f5', fontWeight: 600 }}>Auto-generated QR</div>
                    </div>
                 </div>
              </div>
          </div>
...
export default function ScrollFeatureSection() {
  ...
  return (
    <section ref={containerRef} style={{ position: 'relative', width: '100%', height: '500vh', background: '#0a0a0c' }}>
        ...
        <div style={{ 
          ...
          <div style={{ marginBottom: 20 }}>
            ...
            >
              ...
              />
              <span style={{ fontSize: 11, fontWeight: 700, color: '#7c5cfc', textTransform: 'uppercase', letterSpacing: '0.16em' }}>
            ...
            <AnimatePresence mode="wait">
              <motion.div
                ...
                style={{
                  ...
                  overflow: 'hidden',
                  boxShadow: `0 80px 150px rgba(0,0,0,0.9), 0 0 100px ${activeColor}10`,
                  position: 'relative',
                  zIndex: 20,
                  flexShrink: 0,
                }}
