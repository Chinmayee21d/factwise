...
const SN_VENDORS = [
    ...
    { id: 'V2', name: 'Vendor B', tag: 'Electronics', score: 91, region: 'IN', tone: '#0d9488' },
    { id: 'V3', name: 'Vendor C', tag: 'Motors', score: 88, region: 'DE', tone: '#7b68ee' },
    { id: 'V4', name: 'Vendor D', tag: 'Tier-2', score: 82, region: 'US', tone: '#f59e0b' },
    { id: 'V5', name: 'Vendor E', tag: 'Forging', score: 79, region: 'IN', tone: '#06b6d4' },
];
...
const STEPS_MENU = [
    ...
    { p: 2, title: 'Dispatch on Platform' },
...
.sn-pill { margin-left: auto; display: inline-flex; align-items: center; gap: 5px;
  ...
  color: #6d28d9; letter-spacing: 0.14em; text-transform: uppercase; }
...
.sn-tile.filled { background: rgba(124,58,237,0.06); border-style: solid;
...
.sn-send { display: flex; align-items: center; gap: 7px; justify-content: center;
  padding: 9px 12px; background: linear-gradient(180deg, #8a7dee 0%, #7c3aed 100%);
...
.sn-split { display: grid; grid-template-columns: 1.05fr 0.85fr; gap: 8px;
  flex: 1; min-height: 0; }
...
.sn-msg.bot .av { width: 16px; height: 16px; border-radius: 50%; background: #0b1322;
...
.sn-ticker { background: #0b1322; border-radius: 9px; padding: 10px 10px; display: flex;
  flex-direction: column; gap: 6px; color: white; min-width: 0; }
...
.sn-svgWrap { background: rgba(255,255,255,0.04); border-radius: 7px; padding: 3px;
...
.sn-crit .chip { padding: 1.5px 6px; border-radius: 99px; background: rgba(255,255,255,0.05);
  color: #cbd5e1; font-family: 'JetBrains Mono', ui-monospace, monospace; font-size: 8px;
...
.sn-foot { padding: 8px 13px 9px; background: #fafbff; border-top: 1px solid #eef1f6;
...
export default function Section32SourceAINegotiate({ isActive = true }: { isActive?: boolean }) {
    ...
    const fillD = pts.length > 1
        ? `${pathD} L ${pts[pts.length - 1].split(',')[0]},${H - 4} L ${pts[0].split(',')[0]},${H - 4} Z`
        : '';
                                    ...
                                    <div className="sn-ticker">
                                        ...
                                        <div className="sn-crit">
                                            <span className={`chip ${critActive ? 'act' : ''}`}>
                                                {critActive ? <SNI.Check s={8} /> : null} Quality A
                                            </span>
                                            <span className={`chip ${critActive ? 'act' : ''}`}>
                                                {critActive ? <SNI.Check s={8} /> : null} Lead ≤14d
                                            </span>
                                            <span className={`chip ${step >= 9 ? 'act' : ''}`}>
                                                {step >= 9 ? <SNI.Check s={8} /> : null} Net-30
                                            </span>
                                        </div>
                                    </div>
                                </div>
