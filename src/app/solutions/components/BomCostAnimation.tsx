...
export default function BomCostAnimation({
  ...
  const [s4Rows,  setS4Rows]    = useState<number>(0); // 0..3
  const [s4Total, setS4Total]   = useState<boolean>(false);
  ...
  useEffect(() => {
    ...
    async function run() {
      while (!cancelRef.current) {
        ...
        setS3Cards(0); setS3Best(false); setS4Rows(0); setS4Total(false);
        ...
        setS4Rows(0); setS4Total(false);
        await sleep(400); if (cancelRef.current) return;
        for (let i = 1; i <= DIFF_CHANGES.length; i++) {
          setS4Rows(i); await sleep(450); if (cancelRef.current) return;
        }
        await sleep(450); if (cancelRef.current) return;
        setS4Total(true); await sleep(2800); if (cancelRef.current) return;
      }
    }
    run();
  ...
  useEffect(() => {
    ...
    if (controlledPhase === 1) {
      ...
      setS4Rows(DIFF_CHANGES.length); setS4Total(true);
    }
  ...
  const STEP_META: Record<number, { tag: string; title: string }> = {
    ...
    4: { tag: 'STEP 04', title: 'Revision Tracking & Diff View' },
  };
        ...
        <div className={'bom-scene ' + (currentStep === 4 ? 'on' : '')}>
          <div className="s4-wrap">
            <div className="s4-head">
              <div className="col-l">
                <span className="vlbl v1">v1</span>
                <span className="vt">previous revision</span>
              </div>
              <div className="col-r">
                <span className="vlbl v2">v2</span>
                <span className="vt">current revision · 3 changes</span>
              </div>
            </div>

            <div className="s4-changes">
              {DIFF_CHANGES.map((d, i) => (
            ...
            <div className={'s4-foot ' + (s4Total ? 'in' : '')}>
              <div className="L">
                <div className="ico"><BI.Git s={14} /></div>
                <div>
                  <div className="lt">Rolled-up cost impact</div>
                  <div className="ls">Diff surfaced <b>before approval</b> — no spreadsheet detective work.</div>
                </div>
              </div>
              <div className="R">
                <span className="old">$482</span>
                <span className="new">$528</span>
